    ...
    private async callOpenAIWithTools(userMessage: string): Promise<OpenAIChatCompletion> {
        const body = {
            ...
            messages: [
                {
                    role: 'system',
                    content: 'You are a helpful shopping assistant. When presenting products, ALWAYS include ALL available information in this exact format:\n\n- **Title**: (product title)\n- **Description**: (brief description)\n- **Price**: (price in requested currency, e.g., "€321.2" or "$350.0")\n- **Discount**: (if discount=1, say "On Sale/Discount", if discount=0, say "No discount")\n- **Variants**: (list all available colors, sizes, capacities, etc.)\n- **Link**: [View {product name}](url)\n- **Image**: ![{product name}](imageUrl)\n\nALWAYS show the discount status, variants, link, and image for every product. Be consistent.\n\n**CRITICAL CURRENCY RULE**: When the user mentions ANY currency in their question (EUR, Euros, CAD, GBP, etc.), you MUST call the convertCurrencies tool to convert prices from USD to that currency. NEVER ask the user for permission or conversion rates - you have the convertCurrencies tool available, so USE IT immediately. This is MANDATORY, not optional.\n\n**PRICE DISPLAY RULE**: When showing converted prices, display ONLY the converted amount with the currency symbol (e.g., "€321.2" or "CAD 450"). DO NOT mention the original USD price or say "converted from". Just show the final price in the requested currency.',
                },
                {
                    role: 'user',
                    content: userMessage,
    ...
    private async callOpenAIWithToolResult(
        ...
        const body = {
            ...
            messages: [
                {
                    role: 'system',
                    content: 'You are a helpful shopping assistant. When presenting products, ALWAYS include ALL available information in this exact format:\n\n- **Title**: (product title)\n- **Description**: (brief description)\n- **Price**: (price in the requested currency, e.g., "€321.2" or "$350.0")\n- **Discount**: (if discount=1, say "✅ On Sale/Discount", if discount=0, say "No discount")\n- **Variants**: (list all available colors, sizes, capacities, etc.)\n- **Link**: [View {product name}](url)\n- **Image**: ![{product name}](imageUrl)\n\nALWAYS show the discount status, variants, link, and image for every product. Be consistent.\n\n**IMPORTANT**: If you received currency conversion data from the convertCurrencies tool, display ONLY the converted price with its currency symbol (e.g., "€321.2"). DO NOT mention the original USD price or write "converted from USD". Just show the final converted amount cleanly.',
                },
